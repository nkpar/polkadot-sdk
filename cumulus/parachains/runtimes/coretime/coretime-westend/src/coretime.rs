...
/// With DAP, revenue is accumulated in the satellite account and will eventually be sent
...
impl CoretimeInterface for CoretimeAllocator {
	...
	fn on_new_timeslice(_timeslice: Timeslice) {
		// Revenue is now accumulated in the DAP satellite account via
		// CoretimeRevenueToSatellite (OnRevenue handler). The satellite pallet will
		// eventually send accumulated funds to AssetHub DAP via XCM.
	}
}
